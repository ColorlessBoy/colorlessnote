...
$$
V^{\pi, \gamma}(s) = \mathbb{E}[r_{t+1} + \ldots + r_{t+T} \vert s_t = s], \forall s \in S \\
Q^{\pi, \gamma}(s,a) = \sum_{s' \in S} P(s' \vert s, a) [ R(s, a, s') + \gamma(s') V^{\pi, \gamma}(s')]
$$
...
- **Theorem 1** (Policy Improvement). Given any policy parameter u, let $$u' = u + \alpha g(u)$$. Then, there exists an $$\epsilon > 0$$ such that, for all positive $$\alpha < \epsilon$$, $$J_\gamma(u') \ge J_\gamma(u)$$. Further, if $$\pi$$ has a tabular representation, then $$V^{\pi_{u'}, \gamma}(s) \ge V^{\pi_u, \gamma}(s)$$ for all $$s \in S$$.
  ...
  **proof**: The key point is
  ...
  In tabular representation, we have $$\sum_{a \in A} \pi_{u}(a \vert s) Q^{\pi_u, \gamma}(s, a) \le \sum_{a \in A}\pi_{u'}(a \vert s) Q^{\pi_u, \gamma}(s, a)$$, which can get $$V^{\pi_u, \gamma} \le V^{\pi_u', \gamma}(s)$$.
...
- From Backward to Forward:
  ...
  ![equ2]({{'/assets/img/off-policy-actor-critic/equ2.png' | prepend: site.baseurl}})

...
Firstly, we denote  $$\bar A(s) = \mathbb{E}_{a \sim \tilde \pi(\cdot \vert  s)}\left[ A_\pi(s,a) \right]$$, then
...
$$
\begin{cases}
\mathbb{E}_{s_t\sim\tilde\pi}[\bar A(s_t)] = P(n_t=0)\mathbb{E}_{s_t \sim \tilde\pi \vert  n_t = 0}[\bar A(s_t)] + P(n_t>0)\mathbb{E}_{s_t \sim \tilde\pi \vert  n_t > 0}[\bar A(s_t)],\\
\mathbb{E}_{s_t\sim\pi}[\bar A(s_t)] = P(n_t=0)\mathbb{E}_{s_t \sim \pi \vert  n_t = 0}[\bar A(s_t)] + P(n_t>0)\mathbb{E}_{s_t \sim \pi \vert  n_t > 0}[\bar A(s_t)],\\
\mathbb{E}_{s_t \sim \tilde\pi \vert  n_t = 0}[\bar A(s_t)] =\mathbb{E}_{s_t \sim \pi \vert  n_t = 0}[\bar A(s_t)].
\end{cases}
$$
...
Note that: we can let $$\pi' = \arg\max_{\pi'} L_{\pi_{old}}(\pi')$$ and $$\pi_{new}(a\vert s) = (1 - \alpha)\pi_{old}(a\vert s) + \alpha \pi'(a\vert s) $$, then $$ \pi_{new} $$ and $$ \pi_{old} $$ are $$\alpha$$-coupled.[^1]
...
>=& P(a \ne \tilde a \vert  s) \mathbb{E}[A^\pi(s,\tilde a) - A^\pi(s,a) \vert (a, \tilde a) \sim (\pi(\cdot \vert s), \tilde \pi(\cdot \vert s)), a \ne \tilde a]\\
>\Rightarrow&
